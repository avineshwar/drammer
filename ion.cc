...
int ION_bulk(int len, std::vector<struct ion_data *> &chunks, int max,
             bool mmap) {
  lowmem = false;
...
void ION_clean_all(std::vector<struct ion_data *> &chunks, int max) {
  ...
  chunks.erase(chunks.begin(),
               chunks.begin() + max); // remove first <max> elements
}
...
void ION_get_hammerable_rows(struct ion_data *chunk) {
  if (chunk->len < (3 * rowsize))
    return;
  if (chunk->mapping == NULL)
    return;
  for (int offset = rowsize; offset < chunk->len - rowsize; offset += rowsize) {
...
void ION_fini(void) { close(ion_fd); }
